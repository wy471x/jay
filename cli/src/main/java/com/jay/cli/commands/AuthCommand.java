...
public class AuthCommand implements Callable<Integer> {
    ...
    @Override
    public Integer call() {
        System.out.println("Usage: jay auth <status|set|get|clear|list|migrate>");
        return 0;
    }
        ...
        public Integer call() {
            try {
                ...
                if (key == null || key.isBlank()) {
                    System.err.println("No API key provided. Use --api-key or --api-key-stdin.");
                    return 1;
                }
                store.set(provider.id(), key);
                System.out.println("API key set for " + provider.id());
                return 0;
            } catch (Exception e) {
    ...
    static class Clear implements Callable<Integer> {
        ...
        public Integer call() {
            try {
                ...
                System.out.println("Cleared API key for " + provider.id());
                return 0;
            } catch (Exception e) {
    ...
    static class Migrate implements Callable<Integer> {
        ...
        @Override
        public Integer call() {
            System.out.println("Migration" + (dryRun ? " (dry-run)" : "") + ": checking for legacy keys...");
            System.out.println("No legacy keys found to migrate.");
            return 0;
        }
    }
}
